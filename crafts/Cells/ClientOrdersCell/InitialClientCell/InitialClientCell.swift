...
class InitialClientCell: UICollectionViewCell {
    ...
    public static var BANK_AGREE: String  = "bank_admin_agree"
    ...
    func configCell(item: OrdersModel) {
        ...
        if payType == Constants.PayMethods.ON_DELIVER {
            ...
            if status == Constants.OrderStatus.BANK_AGREE {
                orderStatus.text = "Pending Admin Agreement".localiz()
            }else {
                orderStatus.text = "Not Confirmed".localiz()
            }
